...
import it.volta.ts.easymask.networking.RequestThreadRunner;
...
public class MaskActivity extends AppCompatActivity {
    private RequestThreadRunner requestThreadRunner;
...
//    private ImageView zoomIn, zoomOut;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loadImage(downloadedImg, UrlHandler.getUrl());
        ...
        btnStats.setOnClickListener(view -> {
            ...
            float ratio = (w * h) / (float)(imgWidth * imgHeight);
            float pxScaled = px / ratio;
            float totPx = (imgWidth * imgHeight);
            float perc = MathUtil.roundDown((pxScaled / totPx) * 100, 1);

            coverage.setText("Coverage: " + perc + "%");
            imageDimens.setText("Image size: " + (int) (imgWidth) + "x" + (int) (imgHeight));
            pixelsOutOf.setText((int) (pxScaled) + " pixels out of " + (int) (totPx));
...
//    };
    ...
    View.OnClickListener onUploadBtnClick = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            ...
            //TODO lol
            fileHandler.writeImageToFile(resizedBitmap, UrlHandler.getImgCode());

            requestThreadRunner = new RequestThreadRunner(MaskActivity.this);
            requestThreadRunner.setImgCode(UrlHandler.getImgCode());
            requestThreadRunner.start();
        }
    };
    }
